    ...
    function animateEntrance() {
        ...
        if (titleRef) {
            ...
            if (isMobile) {
                ...
                const normalizedTitle = titleRef.innerText.replace(/\s+/g, " ").trim();
                const chars = normalizedTitle
                    .split("")
                    .map((c) => `<span>${/\s/.test(c) ? "&nbsp;" : c}</span>`)
                    .join("");
    ...
    {:else}
        <header class="header">
            ...
            <h1 class="cinematic-title" bind:this={titleRef}>
                JURNAL HARIAN<br class="mobile-line-break" />RAMADHAN
            </h1>
