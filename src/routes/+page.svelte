<script lang="ts">
  ...
  const IDENTITY_API_URL = 'https://smeduverse.salju.test/api/identity/student';
  ...
  async function lookupStudent() {
    ...
    try {
      const response = await fetch(`${IDENTITY_API_URL}?nis=${encodeURIComponent(nis.trim())}`);
      const payload: unknown = await response.json();

      if (!response.ok) {
        studentFound = false;
        message = { text: getPayloadMessage(payload, 'Data siswa tidak ditemukan'), type: 'error' };
      ...
      if (typeof payload !== 'object' || payload === null) {
        studentFound = false;
        message = { text: 'Format data siswa tidak valid', type: 'error' };
        shakeCard(identityCardRef);
        return;
      }
      ...
      const studentFullname = typeof student.fullname === 'string' ? student.fullname : '';
      const studentNis = typeof student.nipd === 'string' ? student.nipd : nis;
      ...
      if (!studentFullname || !studentRombel) {
        studentFound = false;
        message = { text: 'Data siswa belum lengkap pada layanan identitas', type: 'error' };
        shakeCard(identityCardRef);
        return;
      }
      ...
      studentFound = false;
      message = { text: 'Gagal terhubung ke layanan identitas siswa', type: 'error' };
      shakeCard(identityCardRef);
    } finally {
  ...
  async function submitSheet() {
    ...
    if (statusPuasa !== 'PENUH' && !alasanTidakPuasa.trim()) {
      message = { text: 'Alasan wajib diisi jika puasa tidak penuh', type: 'error' };
    ...
    try {
      const response = await fetch('/api/sheet', {
        ...
        body: JSON.stringify({
          ...
          alasan_tidak_puasa: statusPuasa === 'PENUH' ? '' : alasanTidakPuasa.trim(),
      ...
      const payload: unknown = await response.json();
      if (!response.ok) {
        message = { text: getPayloadMessage(payload, 'Gagal menyimpan laporan'), type: 'error' };
        shakeCard(reportCardRef);
        return;
      }
      ...
      message = { text: 'Terjadi kesalahan saat mengirim laporan', type: 'error' };
    } finally {
      submitLoading = false;
    }
  }
  ...
  function animateEntrance() {
      ...
      if (titleRef) {
          const chars = titleRef.innerText.split('').map(c => `<span>${c}</span>`).join('');
          titleRef.innerHTML = chars;
          const spans = titleRef.querySelectorAll('span');
          
          tl.fromTo(spans, 
             { opacity: 0, y: 20 },
             { opacity: 1, y: 0, stagger: 0.03, duration: 0.8, ease: 'back.out(1.7)' }
          );
      }

      if (formContainerRef) {
          tl.fromTo(formContainerRef.children,
             { opacity: 0, y: 30 },
        ...
        <section class="completion-view" bind:this={completionCardRef}>
            ...
            <div class="completion-content">
                ...
                <div class="quote-card stagger-item">
                    <p class="arabic">فَمَن يَعْمَلْ مِثْقَالَ ذَرَّةٍ خَيْرًا يَرَهُ</p>
                    <p class="translation">"Barang siapa yang berbuat kebaikan sekecil apapun, maka baginya ada pahalanya."</p>
                    <span class="source">(QS. Al-Zalzalah: 7)</span>
                </div>
                <div class="actions stagger-item">
                    <button class="btn-secondary" onclick={resetForm}>Isi Lagi</button>
                    <button class="btn-primary" onclick={goToHome}>Kembali</button>
                </div>
            </div>
        </section>
    {:else}
        <header class="header">
            ...
            <h1 class="cinematic-title" bind:this={titleRef}>Ramadhan Sheet Harian</h1>
        ...
        <div class="form-container" bind:this={formContainerRef}>
            ...
            <article class="glass-card identity-card" bind:this={identityCardRef}>
                ...
                <div class="input-group">
                    ...
                    <div class="search-row">
                        <input
                            ...
                            bind:value={nis}
                            disabled={studentFound || identityLoading || submitLoading}
                            class:has-value={nis.length > 0}
                        />
                        {#if studentFound}
                            <button class="btn-icon" onclick={resetIdentity} title="Ganti Siswa">
            ...
            {#if studentFound}
                <article class="glass-card report-card" bind:this={reportCardRef}>
                    <div class="card-header">
                    ...
                    <div class="form-section">
                        <label class="section-label" for="tanggal">Tanggal Laporan</label>
                        <input id="tanggal" type="date" bind:value={tanggal} class="date-input" />
                    </div>
                        ...
                        <div class="grid-options">
                            {#each fardhuOptions as option}
                                <button
                                    ...
                                    onclick={() => (sholatFardhu = toggleItem(sholatFardhu, option))}
                                >
                                    {option}
                                </button>
                            {/each}
                    ...
                    <fieldset class="form-section">
                        ...
                        {#if statusPuasa !== 'PENUH'}
                            <div class="reason-box">
                                <label for="alasan">Alasan tidak puasa penuh</label>
                                <textarea
                                    id="alasan"
                        ...
                        <div class="grid-options">
                            {#each sunnahOptions as option}
                                <button
                                    ...
                                    onclick={() => (ibadahSunnah = toggleItem(ibadahSunnah, option))}
                                >
                                    {option}
                                </button>
                            {/each}
                    ...
                    <div class="form-section">
                        <label class="section-label" for="tadarus">Tadarus Al-Qur'an</label>
                        <input id="tadarus" type="text" bind:value={tadarus} placeholder="Juz / Surah / Ayat" class="text-input" />
                    </div>

                    <fieldset class="form-section">
                        <legend class="section-label">7 Kebiasaan Anak Indonesia Hebat</legend>
                        <div class="chips-container">
                            {#each kebiasaanOptions as option}
                                <button
                                    ...
                                    onclick={() => (kebiasaan = toggleItem(kebiasaan, option))}
                                >
                                    {option}
                                </button>
                            {/each}
                    ...
                    <div class="action-footer">
                        <button class="btn-submit" type="button" onclick={submitSheet} disabled={submitLoading}>
                            <span class="btn-text">{submitLoading ? 'MENYIMPAN...' : 'SIMPAN LAPORAN'}</span>
...
<style>
  ...
  @media (max-width: 600px) {
      ...
      .puasa-options { grid-template-columns: 1fr; }
